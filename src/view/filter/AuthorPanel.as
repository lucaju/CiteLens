package view.filter {
	...
	public class AuthorPanel extends AbstractPanel {
		 ...
		 * 
		 */
		public function AuthorPanel(source:FilterPanels) {
			super(source);
		}
		...
		override public function init(active:Boolean):void {
			...
			if (active) hasSelectedOptions = CiteLensController(this.getController()).filterHasSelectedOptions(source.filterID, "author");
			
			//label
			this.buildLabel("author");
			...
			if (hasSelectedOptions) {
				var selectedAuthors:Array = CiteLensController(this.getController()).getFilterOptionsByType(source.filterID, "author");
		...
		public function addAuthor(authorName:String = null, focusValue:Boolean = false):void {
			...
			if (!blankSpace) {
				...
				FilterPanels(this.parent).moveBoxes(this, author.height + 3);
