	...
	final public class FunctionBox extends OptionBox {
		...
		override public function selectedOptions():Array {
			...
			for each (var item:Object in optionArray) {																					//Loop in the functions
				...
				//Test if the function has subfunctions
				if (functionOption.hasSubFuntions) {																	//If the function has subfunctions
					
					//Get subfunctions in the current function
					var subFunctionOptions:Array;
					var subFunctionOption:FunctionOption;																					//Function Option: Recreate the option for procesing 
					
					
					//test if the function is expanded.
					if (functionOption.expanded) {
						...
						for each(subItem in subFunctionOptions) {
							...
							if (subFunctionOption.expanded) {
								...
								itemOptions = subFunctionOption.options;																		//Get the option Array in the subFunction
				...
				} else if (functionOption.hasOption) {																												//If the function does't have subfunctions
					
					//Get the options Array
					if (functionOption.expanded) {
						itemOptions = functionOption.optionArray;
					} else {
						itemOptions = functionOption.options;
					}
					...
					if (functionOption.selected) {
						citationFunction.value = functionOption.selected;
					}
				}
